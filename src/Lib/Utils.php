...
namespace Etech\Sms\Lib;

use Etech\Sms\Console\BackgroundProcess;
use Etech\Sms\Constants;
use Etech\Sms\Exception\EtechSmsException;
use Etech\Sms\Message;
...
class Utils
{
    ...
    {
        $bRet = ($oNumberProto = self::getNumberProto($xTel, $sCcode)) &&
            self::phoneUtil()->isValidNumber($oNumberProto) && !empty(self::phoneUtil()->getNumberType($oNumberProto));
    ...
    public static function doBulkSms(array $hData, array $hCredentials, array $hCallBack = [], $oMessage = null): array
    {
        ...
        $defaultCallBack = ['bulk_chunk' => Constants::SMS_MAX_RECIPIENTS];
        ...
        foreach ($asDestinationNumbers as $xNumber) {
            ...
            try {
                ...
                $oMessage = null === $oMessage ?
                    Message::create($hCredentials['api_key'], $hCredentials['api_secret']) : $oMessage;
                ...
                @trigger_error('ERROR occurred during sending SMS to' . $xNumber, E_USER_WARNING);
                continue;
            }
    ...
    public static function backgroundProcess(array $hData, array $hCredentials, array $hCallBack = [])
    {
        ...
        if ($hCallBack['path_to_php'] === 'php' || is_executable($hCallBack['path_to_php'])) {
            if (file_put_contents(Constants::getSMSPath() . 'tmp/' . $sTmpName, json_encode($hData) . PHP_EOL, LOCK_EX)) {
                $sBIN = $hCallBack['path_to_php'] . ' -f ' . Constants::getSMSPath() . 'bin/camoo.php';
