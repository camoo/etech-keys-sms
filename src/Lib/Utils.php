...
class Utils
{
    ...
    {
        $asPersonlizeMsgFind = \Etech\Sms\Constants::PERSONLIZE_MSG_KEYS;
        $iCount = 0;
        $axMsgSent = [];
        $batch_loop = 2;
        $iBatch = 1;
        ...
        $defaultCallBack=['bulk_chunk' => \Etech\Sms\Constants::SMS_MAX_RECIPIENTS];
    ...
    public static function backgroundProcess(array $hData, array $hCredentials, array $hCallBack=[])
    {
        ...
        if ($hCallBack['path_to_php'] === 'php' || is_executable($hCallBack['path_to_php'])) {
            if (file_put_contents(\Etech\Sms\Constants::getSMSPath(). 'tmp/' .$sTmpName, json_encode($hData).PHP_EOL, LOCK_EX)) {
                $sBIN = $hCallBack['path_to_php'] .' -f '. \Etech\Sms\Constants::getSMSPath(). 'bin/camoo.php';
